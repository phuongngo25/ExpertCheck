...
def process_wyoflex_data():
    ...
    logger.info(">>> BẮT ĐẦU PIPELINE TÁCH VÀ XỬ LÝ DỮ LIỆU WYOFLEX (TAY PHẢI/TRÁI) <<<")
    
    try:
        config = get_config()
        os.makedirs(config.paths.processed_data_dir, exist_ok=True)

        #  2 file cho tay phải và tay trái 
        df_right, df_left = _group_and_process_raw_files(config)
        
        # Lưu file cho tay phải
        if df_right is not None and not df_right.empty:
            right_output_path = os.path.join(config.paths.processed_data_dir, config.paths.wyoflex_emg_csv_right)
            df_right.to_csv(right_output_path, index=False)
            logger.info(f"Đã lưu dữ liệu TAY PHẢI tại: '{right_output_path}'")
        else:
            logger.warning("Không tìm thấy dữ liệu cho tay phải.")

        # Lưu file cho tay trái
        if df_left is not None and not df_left.empty:
            left_output_path = os.path.join(config.paths.processed_data_dir, config.paths.wyoflex_emg_csv_left)
            df_left.to_csv(left_output_path, index=False)
            logger.info(f"Đã lưu dữ liệu TAY TRÁI tại: '{left_output_path}'")
        else:
            logger.warning("Không tìm thấy dữ liệu cho tay trái.")
...
def _group_and_process_raw_files(config):
    logger = logging.getLogger("WyoFlexProcessor")
    ...
    logger.info("Đang quét và nhóm các file...")
    
    for root, _, files in os.walk(voltage_dir):
        for filename in files:
            # Tối ưu: Chỉ xử lý các file không có offset (O2)
            if 'O1' in filename:
                continue
            ...
            try:
                # Tạo key bằng cách bỏ đi mã sensor S#
                trial_key = re.sub(r'S\d+', '', filename)
                trial_files[trial_key].append(os.path.join(root, filename))
            except (AttributeError, IndexError):
                logger.warning(f"Bỏ qua file có tên không đúng định dạng: {filename}")
    ...
    for key, files in tqdm(trial_files.items(), desc="Đang xử lý các trials"):
        ...
        try:
            ...
            # Phân loại vào DataFrame của tay phải hoặc tay trái
            if f_id == 1:
